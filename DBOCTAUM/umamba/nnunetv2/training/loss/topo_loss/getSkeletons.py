    ...
    for i, lbp in enumerate(label_paths):
        if lbp.split('.')[-1] in ['png']:
            ...
            if cnt == 1:
                ...
                plt.subplot(2, 1, 1), plt.imshow(skelen[:, :, 0], cmap='gray'), plt.colorbar(), plt.title(
                    'back_skelen')
                plt.subplot(2, 1, 2), plt.imshow(skelen[:, :, 1], cmap='gray'), plt.colorbar(), plt.title(
                    'fore_skelen')
