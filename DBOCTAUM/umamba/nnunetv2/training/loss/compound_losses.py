import torch
from nnunetv2.training.loss.dice import SoftDiceLoss, MemoryEfficientSoftDiceLoss, SoftSkeletonRecallLoss, SoftDicePlusLoss, MemoryEfficientSoftDicePlusLoss
...
class DC_plus_and_CE_loss(nn.Module):
    def __init__(self, soft_dice_kwargs, ce_kwargs, weight_ce=1, weight_dice=1, ignore_label=None,
                 dice_class=SoftDicePlusLoss):
        """
        Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        :param soft_dice_kwargs:
        :param ce_kwargs:
        ...
        :param weight_dice:
        """
        super(DC_plus_and_CE_loss, self).__init__()
        if ignore_label is not None:
            ce_kwargs['ignore_index'] = ignore_label
...
class SkelREC_loss(nn.Module):
    def __init__(self, soft_skelrec_kwargs, weight_srec=1, ignore_label=None):
        """
        Weights for CE and Dice do not need to sum to one. You can set whatever you want.
        :param soft_dice_kwargs:
        :param soft_skelrec_kwargs:
        ...
        :param weight_dice:
        """
        super(SkelREC_loss, self).__init__()
    ...
    def forward(self, net_output: torch.Tensor, target: torch.Tensor, skel: torch.Tensor):
        ...
        result = self.weight_srec * srec_loss
        return result
