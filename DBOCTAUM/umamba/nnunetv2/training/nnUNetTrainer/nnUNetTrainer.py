...
from nnunetv2.training.loss.compound_losses import DC_and_CE_loss, DC_and_BCE_loss, DC_plus_and_CE_loss, \
    DC_SkelREC_and_CE_loss, SkelREC_loss
from nnunetv2.training.loss.deep_supervision import DeepSupervisionWrapper
from nnunetv2.training.loss.dice import get_tp_fp_fn_tn, MemoryEfficientSoftDiceLoss, MemoryEfficientSoftDicePlusLoss
...
class nnUNetTrainer(object):
    def __init__(self, plans: dict, configuration: str, fold: int, dataset_json: dict, unpack_dataset: bool = True,
        ...
        self.skelrec_loss = SkelREC_loss(soft_skelrec_kwargs={'batch_dice': self.configuration_manager.batch_dice,
                                                              'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp},
                                         weight_srec=self.weight_srec, ignore_label=self.label_manager.ignore_label, )
        self.loss_skelrec = False
        self.loss_dicePlus = False
    ...
    def _build_loss(self):
        if self.loss_dicePlus:
            loss = DC_plus_and_CE_loss({'batch_dice': self.configuration_manager.batch_dice,
                                        'smooth': 1e-5, 'do_bg': False, 'ddp': self.is_ddp}, {}, weight_ce=1,
                                       weight_dice=1,
                                       ignore_label=self.label_manager.ignore_label,
                                       dice_class=MemoryEfficientSoftDicePlusLoss)
    ...
    def do_split(self):
        """
        The default split is a 10 fold CV on all available training cases. nnU-Net will create a split (it is seeded,
    ...
    def get_dataloaders(self):
        ...
        tr_transforms = self.get_training_transforms(
            ...
            ignore_label=self.label_manager.ignore_label, loss_skelrec=self.loss_skelrec)
        ...
        val_transforms = self.get_validation_transforms(deep_supervision_scales,
                                                        ...
                                                        ignore_label=self.label_manager.ignore_label,
                                                        loss_skelrec=self.loss_skelrec)

        if self.loss_skelrec:
    ...
    def get_training_transforms(
        ...
        if loss_skelrec:
            tr_transforms.append(SkeletonTransform(do_tube=True))
        ...
        # np-->tensor
        if loss_skelrec:
            tr_transforms.append(NumpyToTensor(['data', 'target', 'skel'], 'float'))
        else:
    ...
    def get_validation_transforms(
        ...
        if loss_skelrec:
            val_transforms.append(SkeletonTransform(do_tube=True))
        ...
        if deep_supervision_scales is not None:
            val_transforms.append(DownsampleSegForDSTransform2(deep_supervision_scales, 0, input_key='target',
                                                               output_key='target'))
        if loss_skelrec:
            val_transforms.append(NumpyToTensor(['data', 'target', 'skel'], 'float'))
        else:
    ...
    def run_training(self):
        ...
        for epoch in range(self.current_epoch, self.num_epochs):
            ...
            for batch_id in range(self.num_iterations_per_epoch):
                if self.loss_skelrec:
                    train_outputs.append(self.train_ske_step(next(self.dataloader_train)))
                else:
            ...
            with torch.no_grad():
                ...
                for batch_id in range(self.num_val_iterations_per_epoch):
                    if self.loss_skelrec:
                        val_outputs.append(self.validation_ske_step(next(self.dataloader_train)))
                    else:
    ...
    #     self.on_train_start()
    #     print('training start!')
    #     heatmaps = np.zeros([320, 320])
    #     self.num_iterations_per_epoch = 1
    #     for batch_id in range(self.num_iterations_per_epoch):
    #         grad = self.eval_step(next(self.dataloader_train))
    #     #     heatmaps = heatmaps + grad.cpu().numpy()
    #     # cam = heatmaps
    #     # cam = cam / cam.max()
    ...
    #     # ax.imshow(cam)
    #     # # cb = fig.colorbar(ax.images[-1], ax=ax)
    #     # ax.axis('off')  # 关闭坐标轴
    #     # plt.show()
    #     # plt.savefig("nn-e-6.png")  # 保存每个图像的热图，或者进行其他处理
    #     print('eval end')
