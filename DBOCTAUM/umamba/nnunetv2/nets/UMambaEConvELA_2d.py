...
from .UNetELA import ELA,MELA, Conv3ELA
...
class ResidualMambaEncoder(nn.Module):
    def __init__(self,
        ...
        mamba_layers = []
        stage_res_layers = []
        stage_umambaConv_layers = []
        attn_ela_layers = []

        for s in range(n_stages):
            stage_res_layers.append( BasicResBlock(
                    conv_op=conv_op,
                    norm_op=norm_op,
                    norm_op_kwargs=norm_op_kwargs,
                    input_channels=input_channels,
            ...
            stage_umambaConv_layers.append(nn.Sequential(*[
                    BasicBlockD(
                        conv_op=conv_op,
            ...
            attn_ela_layers.append(Conv3ELA(features_per_stage[s]))
            # attn_ela_layers.append(MELA(features_per_stage[s]))
        ...
        self.stage_res = nn.ModuleList(stage_res_layers)
        self.mamba_conv = nn.ModuleList(stage_umambaConv_layers)
        ...
        self.ela_layers = nn.ModuleList(attn_ela_layers)

    def forward(self, x):
        if self.stem is not None:
            x = self.stem(x)
        ret = []
        for s in range(len(self.stage_res)):
            x = self.stage_res[s](x)
            x_ela =  self.ela_layers[s](x)#uela
            x = self.mamba_conv[s](x)
            x = self.mamba_layers[s](x)
            # x = x + x_ela
            # x = 0.4 * x_ela + 0.6 * x #ELA4
            # x = 0.6 * x_ela + 0.4 * x #ELA6
            x = 0.2 * x_ela + 0.8 * x #ELA2
            # x = 0.8 * x_ela + 0.2 * x  # ELA8
            # x = 0.8 * x_ela + 0.2 * x  # ELA8
            # x = 1.4 * x_ela + 0.6 * x  # ELA14
            # x = 0.6 * x_ela + 1.4 * x  # ELA06_14
            ret.append(x)
...
def get_umamba_enc_2d_from_plans(
    ...
    segmentation_network_class_name = 'UMambaEAttn'
    network_class = UMambaEAttn
    kwargs = {
        'UMambaEAttn': {
